...
/* version ok le 3/02/2026  message> 128 car */
...
#sBufferTrace:  .skip 1000
#sBufferTrace1:  .skip 1000
sEp0_buf:      .skip 160
/**********************************...-..--**/
/* SECTION CODE                              */
/**********************************************/
...
/**********************************************/ 
 ep3InHandler:                      # INFO: ep3InHandler
   ...
   # call usbgetendpointconfiguration
    la a0,cfg_endpoints
    addi a0,a0,uec_fin * 3
    
    li a1,0            # message vide
    li a2,64           # obligatoire
    li a3,0            # top message
    call usbstarttransfert   
 100:    
    lw      ra, 0(sp)
...
2:
   ...
   # call usbgetendpointconfiguration
    ...
    li a3,1                # top message
    call usbstarttransfert
   # li a0,2                # TODO: voir plus petit (attente courte) 
   # call attendre 
   ...
   # call usbgetendpointconfiguration
    ...
    li a3,1               # top message
    call usbstarttransfert
   # li a0,1                # TODO: voir plus petit (attente courte) 
   # call attendre 
...
6:
    ...
    li a0,EP3_IN_ADDR     # oui envoi message début
    call usbgetendpointconfiguration
    li a1,0
    li a2,0
    li a3,0
...
usbstarttransfert:       # INFO: usbstarttransfert
    ...
    mv s0,a0
1:
    li t2,64                # taille maxi d'un paquet ?
...
# we're going to leave it clear.
    ...
    #li a0,0xA000
    #csrr a1, RVCSR_MEINEXT_OFFSET # N° exception voir tableau 3.8.4.1. Exceptions
    #csrr a2,0x342
...
appelsoft1:                      # INFO: appelsoft1
   ...
   # mv a1,t0
   # mv a2,t2
    #call majScratchRep         # si erretur à remettre puis voir powman
                                # avec E:\Pico2\projetARM12\build affpowman.uf2
                                # et remettre le watchdog
    li a0,25
    call initGpioLed
    li a0,10
    call ledEclats
